...
public class Nile {
  ...
  public Nile(Hashtable<String, Device> p, LinkedList<String> td,
     ...
     DataOutputStream out, out3,out2= new DataOutputStream(new FileOutputStream("boogy"));;
      if(!dFlood)  {      
        ...
        out3 = new DataOutputStream(new FileOutputStream("result/" + trace + "/"
            + bandwidth + "NilebwVolume"+Integer.toString(kfact)+Boolean.toString(enc)+".txt"));
      }
      else{
        out = new DataOutputStream(new FileOutputStream("result/" + trace + "/"
            + bandwidth + "DirectedFloodVolume.txt")); 
        out3 = new DataOutputStream(new FileOutputStream("result/" + trace + "/"
            + bandwidth + "DirectedFloodbwVolume.txt"));      
      }   
  ...
  public void assignHubs(String trace){
    BufferedReader in;
    PossibleHop hubtemp;
    LinkedList<PossibleHop>hubList = new LinkedList<PossibleHop>();
    try{      
      in = new BufferedReader(new FileReader("resource/" + trace + "/summary3")); 
      int i=0;
      while(in.ready()){
        String line =in.readLine();
        String []numbers=line.split(" ");
         hubtemp = new PossibleHop();
        hubtemp.index= i++;
        hubtemp.value = Double.parseDouble(numbers[0]);
        hubtemp.AvgDtime = Double.parseDouble(numbers[1]);
        hubtemp.AvgSize = Double.parseDouble(numbers[2]);
    ...
    double tmeetingMax=0,dmeetingMax=0,onlinetimeMax=0;
    for(int i=0;i<hubList.size();i++){
      hubtemp = hubList.get(i);
      if(tmeetingMax<hubtemp.value)
        tmeetingMax= hubtemp.value;
      if(dmeetingMax<hubtemp.AvgDtime)
        dmeetingMax= hubtemp.AvgDtime;
      if(onlinetimeMax<hubtemp.AvgSize)
        onlinetimeMax= hubtemp.AvgSize;
    }
    
    for(int i=0;i<hubList.size();i++){
      hubtemp = hubList.get(i);
      hubtemp.VAvgDtime = (hubtemp.value/tmeetingMax)*(hubtemp.AvgDtime/dmeetingMax)*(hubtemp.AvgSize/onlinetimeMax);
    }
    Collections.sort(hubList, new Compositeonlinetime());
    
  }
  public class Compositeonlinetime implements Comparator {

    public int compare(Object o1, Object o2) {
          
      return (int)(((PossibleHop)o1).VAvgDtime-((PossibleHop)o2).VAvgDtime);
    }
  }
  ...
  public void MyAlgo_rout(Msg mtemp, boolean applyDij, Date histT, DataOutputStream out) {  
    ...
    TSlot ttemp;    
    {
      ...
      float cogProbability = getSpectrumProbab(itime, mtemp.ID);  
  ...
  public float getSpectrumProbab(TSlot itime, Integer mID){
    Apoint aptemp = gApTable.get(itime.apname);
